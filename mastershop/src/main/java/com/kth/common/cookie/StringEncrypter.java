...
public class StringEncrypter {
	...
	private static final String passwd = "12345^&*()ABCDEFG"; // 기본 암호
	private static final String iv = "#####&&&!!!&&&#####"; // 초기화 백터

	/**
	 * 기본 암호와 초기화 벡터를 선언된 상수를 사용하도록 변경
	 * 
	 * @throws Exception
	 */
	...
	public StringEncrypter() throws Exception {
		...
		this.key = new SecretKeySpec(md5.digest(passwd.getBytes("UTF8")), "AES");
		this.initalVector = new IvParameterSpec(md5.digest(iv.getBytes("UTF8")));
	}
	 ...
	 * 
	 * @param key
	 *            A key string which is converted into UTF-8 and hashed by MD5.
	 *            Null or an empty string is not allowed.
	 * @param initialVector
	 *            An initial vector string which is converted into UTF-8 and
	...
	public StringEncrypter(String key, String initialVector) throws Exception {
		...
		this.initalVector = new IvParameterSpec(md5.digest(initialVector
				.getBytes("UTF8")));
	}
	 ...
	 * 
	 * @param value
	 *            A string to encrypt. It is converted into UTF-8 before being
	 ...
	 * 
	 * @param value
	 *            A string to decrypt. It must be a string encrypted with the
	...
	public String decrypt(String value) throws Exception {
		if (value == null || value.equals(""))
			throw new Exception(
					"The cipher string can not be null or an empty string.");
