...
if (keycnt == 0)
{
	top.right_frame.document.all.authkey.value = "";
}
keycnt =1;
}
...
if (idcnt == 0)
{
	top.right_frame.document.all.loginID.value = "";
}
idcnt = 1;
}
...
if (passcnt == 0)
{
	top.right_frame.document.all.loginPWD.value = "";
}
passcnt = 1;
}
...
//OPEN_API 버전 확인 함수
function getver(){
	top.right_frame.document.getElementById("verBox").innerText = KTOpenAPIX.GetApiVer();
}
...
function Login(){
	var	result		=	500;
	var Server		=	top.right_frame.document.all.server.value;
	var Authkey		=	top.right_frame.document.all.authkey.value;
	var LoginID		=	top.right_frame.document.all.loginID.value;
	var LoginPWD	=	top.right_frame.document.all.loginPWD.value;
	var date = new Date();
	...
	try {
		...
		/*	// 2014-07-25 - 창영 : 처리를 외부에서 하기 위하여 & 참고용으로 주석처리 
			switch (result)
			{
		...
		top.right_frame.document.getElementById("sSendDate").value = time;
		
		top.right_frame.openApiloginResult(result);

	}catch(ex){
		alert("통화 Open API 사용을 위해 파일 설치가 필요합니다.\n\nKTOpenAPI.exe 파일을 실행하여 주세요.");
		location.href = "/resources/down/KTOpenAPI.exe";
	}
}
...
function  Logout(){
	var result = 500;
	result = confirm("통화오픈API를 로그아웃 하시겠습니까?");
	if (result == true)
	{
		KTOpenAPIX.Logout();

		top.left_frame.document.leftForm.checkApi.value = "LOGOUT";
		
		//로그아웃시 자동재로그인 방지
		$("#fd_openapi_autologin", parent.frames["right_frame"].document).val('F');
		//top.right_frame.document.getElementById("fd_openapi_autologin").value = "F";
		
		//$("#chk_autologin").attr("checked",false);
		//$(window.frames["right_frame"].document).find("#chk_autologin").attr("checked",false);
		top.right_frame.document.getElementById("chk_autologin").checked = false;
		...
		//$(window.frames["right_frame"].document).find("#id_link_openapi_logout").hide();
		//$(window.frames["right_frame"].document).find("#id_link_openapi_login").show();
		top.right_frame.document.getElementById("id_link_openapi_logout").style.display = "none";
		top.right_frame.document.getElementById("id_link_openapi_login").style.display = "";
		
		
		//통화오픈api 로그아웃클릭시 DB도 자동로그인 풀림업데이트
		//openapi user info 업데이트
		var frm = top.right_frame.document.openApiLoginForm;
...
if( KTDPhone != null && KTDPhone != undefined )
	...
	var KTDPhoneArray	=	( new VBArray( KTDPhone ) ).toArray();
		...
		top.right_frame.document.getElementById("GetPhoneList").value = GetPhoneList;
}
...
function GetMobilePhoneList()
{
	...
	top.right_frame.document.getElementById("GetMobilePhoneList").value = MobilePhoneInfo;
}
...
function SetLineState(){
	var result = 500;
	var sTelNum = top.right_frame.document.all.sTelNum.value;
	var sTelState = top.right_frame.document.all.sTelState.value;
	var sRecvTel = top.right_frame.document.all.sRecvTel.value;
...
function SetRecvPhone(){
	var	result		=	500;
	var sRecvPhone = top.right_frame.document.all.sRecvPhone.value;
		result = KTOpenAPIX.SetRecvPhone(sRecvPhone)
		
		switch (result)
		{
		case 0: alert("통화오픈API를 재로그인 해보기시 바랍니다. 문제가 해결되지 않을경우 고객센터로 문의바랍니다."); break;
		case 200: break;
...
function GetRecvPhone(){
	...
	if( GetPhone != null && GetPhone != undefined )
		...
		top.right_frame.document.getElementById("RecvPhonelist").value =RecvPhonelist;	
}
...
function SendCTC(){
	var	result		=	500;
		var sCaller	=	top.right_frame.document.all.sCaller.value;
		var sCallee	=	top.right_frame.document.all.sCallee.value;

		sCaller = sCaller.split("-").join("");
		sCallee = sCallee.split("-").join("");
		result	=	KTOpenAPIX.SendCTC(sCaller, sCallee);
		
		switch (result)
		{
		case 0: alert("통화 Open API를 재로그인 해보기시 바랍니다. 문제가 해결되지 않을경우 고객센터로 문의 바랍니다."); break;
		case 200: break;
...
function SendCONAddUser()
	{
		...
		var sCaller		=	top.right_frame.document.all.sCaller.value;
		var sCallee	=	top.right_frame.document.all.sCallee.value;
		var sDBID		=	top.right_frame.document.all.sDBID.value;
...
function SendSMS(){
		var	result		=	500;
		var MsCaller		=	top.right_frame.document.all.MsCaller.value;
		var sDisplay	=	top.right_frame.document.all.sDisplay.value;
		var sMessage		=	top.right_frame.document.all.sMessage.value;
...
function SendReserveSMS(){
		...
		var sDisplay		=	top.right_frame.document.all.sDisplay.value;
		var sMessage		=	top.right_frame.document.all.sMessage.value;
		var sSendDate		=	top.right_frame.document.all.sSendDate.value;
		var ReserveSMS	 ="";
		var list		= "";
	...
	top.right_frame.document.getElementById("SendReserveSMS").value =ReserveSMS;	
}
// 예약문자 수정 요청 함수
function EditReserveSMS(){
		var	result		=	500;
		var MsCaller		=	top.right_frame.document.all.MsCaller.value;
		var sSmsGroupSeqNo 	=	top.right_frame.document.all.sSmsGroupSeqNo.value;
		var sDisplay		=	top.right_frame.document.all.sDisplay.value;
		var sMessage		=	top.right_frame.document.all.sMessage.value;
		var sSendDate		=	top.right_frame.document.all.sSendDate.value;
		var ReserveSMS	 ="";
...
function GetAbsenceCallList(){
		...
		top.right_frame.document.getElementById("GetAbsenceCallList").value =CallList;	
}
//통화 내역 수 요청 함수
function GetCallCount(){
	var sKind		=	top.right_frame.document.all.sKind.value;
	var kind	 =	"";
	switch (sKind)
	{
...
function GetCallList(){
	var sKind		=	top.right_frame.document.all.sKind.value;
	var nStart		=  top.right_frame.document.all.nStart.value;
	var nCount	=  top.right_frame.document.all.nCount.value;
	var CallList = "";
...
function GetCall(){
	...
	var CallsDBID = top.right_frame.document.all.CallsDBID.value;
...
function GetAbsenceSmsList()
{
	...
	var KTDCallDataArray	=	( new VBArray( KTDCallData ) ).toArray();
		...
		top.right_frame.document.getElementById("GetAbsenceSmsList").value =GetSmsList;	
}

//문자 내역 수 요청 함수
function GetSmsCount()
{
	var SmssKind = top.right_frame.document.all.SmssKind.value;
...
{	
	var GetSmsList	= ""; 
	var SmssKind	=	top.right_frame.document.all.SmssKind.value;
	var SmsnStart	=	top.right_frame.document.all.SmsnStart.value;
	var SmsnCount	=	top.right_frame.document.all.SmsnCount.value;
...
function GetSms()
{
	var SmssDBID = top.right_frame.document.all.SmssDBID.value;
...
function GetCallMemoCount(){
		var cCmType = top.right_frame.document.all.cCmType.value;
		var type ="";
		if (cCmType == "1")
		{
			type="개인 통화 메모 갯수 : ";
...
function GetCallMemoList(){
		...
		var MemonStart = top.right_frame.document.all.MemonStart.value;
		var MemonCount = top.right_frame.document.all.MemonCount.value;
		var MemoList ="";
	...
	if( KTDCallData != null && KTDCallData != undefined )
		...
		top.right_frame.document.getElementById("GetCallMemoCount").value =MemoList;	
}

//통화 메모 내역 요청 함수 
function GetCallMemo(){
		var cCmType = top.right_frame.document.all.cCmType.value;
		var MemosDBID  = top.right_frame.document.all.MemosDBID  .value;
...
function NewCallMemo(){
		var	result		=	500;
		var cCmType = top.right_frame.document.all.cCmType.value;
		var sCLDBID = top.right_frame.document.all.sCLDBID.value;
		var sCallName = top.right_frame.document.all.sCallName.value;
		var sCallGroup = top.right_frame.document.all.sCallGroup.value;
		var sProgGroup = top.right_frame.document.all.sProgGroup.value;
		var sMemo = top.right_frame.document.all.sMemo.value;
...
function GetAddressGroupList(){
		var cAgType = top.right_frame.document.all.cAgType.value;
		var sPDBID = top.right_frame.document.all.sPDBID.value;
		var GroupList ="";
	...
	if( KTDCallData != null && KTDCallData != undefined )
		...
		top.right_frame.document.getElementById("GetAddressGroupList").value =GroupList;	
}

//주소록 그룹 정보 가져오기 요청 함수
function GetAddressGroup(){
		var cAgType = top.right_frame.document.all.cAgType.value;
		var GroupsDBID = top.right_frame.document.all.GroupsDBID.value;
...
function NewAddressGroup(){
		var result = 500;
		var cAgType = top.right_frame.document.all.cAgType.value;
		var sAgName = top.right_frame.document.all.sAgName.value;
		var sPDBID = top.right_frame.document.all.sPDBID.value;
...
function GetAddressDataList(){
		var DatacAgType = top.right_frame.document.all.DatacAgType.value;
		var DatasPDBID  = top.right_frame.document.all.DatasPDBID.value;
	...
	if( KTDCallData != null && KTDCallData != undefined )
		...
		top.right_frame.document.getElementById("GetAddressDataList").value =DataList;	
...
function GetAddressData(){
			var DatacAgType = top.right_frame.document.all.DatacAgType.value;
			var DatasDBID  = top.right_frame.document.all.DatasDBID.value;
			...
			var	KTDAddressData   =	KTOpenAPIX.GetAddressData(DatacAgType, DatasDBID);
				...
				top.right_frame.document.getElementById("sAdName").value = KTDAddressData.Name;	
				top.right_frame.document.getElementById("sAdAddress").value = KTDAddressData.Address;
				top.right_frame.document.getElementById("sAdBirthDay").value = KTDAddressData.BirthDay;
				top.right_frame.document.getElementById("cAdBirthType").value = KTDAddressData.BirthType;
				top.right_frame.document.getElementById("sAdBusiness").value = KTDAddressData.Business;
				top.right_frame.document.getElementById("sAdCompany").value = KTDAddressData.CompanyName;
				top.right_frame.document.getElementById("sAdTeam").value = KTDAddressData.Department;
				top.right_frame.document.getElementById("sAdEmail").value = KTDAddressData.Email;
				top.right_frame.document.getElementById("sAdFNumber").value = KTDAddressData.FaxNum;
				top.right_frame.document.getElementById("sAdFavorite").value = KTDAddressData.Favorite;
				top.right_frame.document.getElementById("sAdHNumber").value = KTDAddressData.HomeNum;
				top.right_frame.document.getElementById("sAdKeyMan").value = KTDAddressData.Keyman;
				top.right_frame.document.getElementById("sAdMemo").value = KTDAddressData.Memo;
				top.right_frame.document.getElementById("sAdMetChange").value = KTDAddressData.MetChance;
				top.right_frame.document.getElementById("sAdMNumber").value = KTDAddressData.MobileNum;
				top.right_frame.document.getElementById("sAdONumber").value = KTDAddressData.OfficeNum;
				top.right_frame.document.getElementById("sAdTitle").value = KTDAddressData.Position;
				top.right_frame.document.getElementById("sAdZipCode").value = KTDAddressData.ZipCode;
...
function NewAddressData(){
			...
			var DatacAgType = top.right_frame.document.all.DatacAgType.value;
			var DatasPDBID  = top.right_frame.document.all.DatasPDBID.value;
			var sAdName  = top.right_frame.document.all.sAdName.value;
			var sAdMNumber  = top.right_frame.document.all.sAdMNumber.value;
			var sAdONumber  = top.right_frame.document.all.sAdONumber.value;
			var sAdHNumber  = top.right_frame.document.all.sAdHNumber.value;
			var sAdFNumber  = top.right_frame.document.all.sAdFNumber.value;
			var sAdCompany  = top.right_frame.document.all.sAdCompany.value;
			var sAdTeam  = top.right_frame.document.all.sAdTeam.value;
			var sAdTitle  = top.right_frame.document.all.sAdTitle.value;
			var sAdBusiness  = top.right_frame.document.all.sAdBusiness.value;
			var sAdZipCode  = top.right_frame.document.all.sAdZipCode.value;
			var sAdAddress  = top.right_frame.document.all.sAdAddress.value;
			var sAdEmail  = top.right_frame.document.all.sAdEmail.value;
			var sAdMemo  = top.right_frame.document.all.sAdMemo.value;
			var sAdBirthDay  = top.right_frame.document.all.sAdBirthDay.value;
			var cAdBirthType  = top.right_frame.document.all.cAdBirthType.value;
			var sNgName  = top.right_frame.document.all.sNgName.value;
			var sAdMetChance  = top.right_frame.document.all.sAdMetChance.value;
			var sAdKeyman  = top.right_frame.document.all.sAdKeyman.value;
			var sAdFavorite  = top.right_frame.document.all.sAdFavorite.value;
